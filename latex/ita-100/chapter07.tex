...
\chapter{March 7}
\paragraph{Topics} 
\textit{Modal verbs, shopping, recap of verbs, \iti{quello} and \iti{questo}, more and less - \iti{più}}

\section{Grammar} \label{0307-Grammar}
...
\centering
    \begin{tabular}{ r l  | r l  | r l  }
         \itb{potere} & \textit{to be able} & \itb{volere} & \textit{to want} & \itb{dovere} & \textit{to have to} \\
         \hline
...
\subsection{This and that: \iti{quello} and \iti{questo}}
We use \textbf{demonstrative adjectives} to show the thing we are talking about: 
`this' or `that' thing under discussion. As adjectives, they change ending to 
match the noun, e.g., `this': \iti{questo}, \iti{questa}, `that': \iti{quello},
\iti{quella}. See \ref{tab:tDemonstrativeAdjectives}.

If \iti{quello} is used before a masculine singular noun, it is shortened to 
\iti{quel}, similarly to how \iti{bello} becomes \iti{bel}. 

\cite{LearnItalianGo} has a good explanation.

\begin{table}[ht] %[ht] % place table roughly [h]ere or else at the [t]op of page
	\centering
	\begin{tabular}{ r  | c c | c c }
		               & \textbf{masc. sing.}        & \textbf{masc. pl.} & \textbf{fem. sing.}  & \textbf{fem. pl.} \\
		this, these    & \iti{questo, quest'}        & \iti{questi}       & \iti{questa, quest'} & \iti{queste}  \\
		that, those    & \iti{quello, quell', quel'} & \iti{quegli, quei} & \iti{quella, quell'} & \iti{quelle}  \\
	\end{tabular}
	\caption{Demonstrative adjectives}
	\label{tab:tDemonstrativeAdjectives}
\end{table}

\begin{itemize}
	\item I'd like that bottle please. \\
	    \iti{Vorrei quella bottiglia, per favore.}
	\item This book. \\
	     \iti{Questo libro.}
	\item I like this sparkling water. \\
	     \iti{Mi piace questa acqua frizzante.}
	\item Claudio is in that store. \\
	     \iti{Claudio è in quel negozio.}
	\item Claudio is in this store. \\
         \iti{Claudio è in questo negozio.}
\end{itemize}

\section{Vocabulary} \label{0307-Vocabulary}
\subsection{Words}
\begin{table}[ht] %[ht] % place table roughly [h]ere or else at the [t]op of page
\centering
    \begin{tabular}{ r l  | r l  | r l  }
...
\subsection{Phrases - speaking about speaking Italian}
...
\subsection{Phrases - more and less}

The word \itb{più} is used with nouns or adjectives to express more-ness. This can go in both 
directions, e.g. `more small' and `more big'. In the other direction, \iti{meno} represents less-ness.
...
\section{Conversation: \iti{Al negozio}} \label{0307-Conversation}
...
\section{Homework} \label{0307-Homework}
\begin{enumerate}
